...
function checkForSubmit() {
    if (
        text.value == "" ||
        !expiration.value ||
        expiration.value < new Date().toISOString().slice(0, 10)
    ) {
        emit("lose-focus");
    } else {
...
<template>
    <li ref="taskContainer" class="task-list-item">
        <div class="task">
            <input ref="textInput" v-model="text" type="text" />
            <label class="due">
                <input
                    ref="dateInput"
                    v-model="expiration"
            ...
            <div></div>
            <SelectTag ref="tagInput" :initial-value="NoTag" />
